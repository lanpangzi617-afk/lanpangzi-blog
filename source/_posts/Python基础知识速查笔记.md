---
title: Python基础知识速查笔记
date: 2026-01-12 15:48:28
categories:
  - python
tags:
  - basic
...
> 用于在一行代码中完成简单的条件判断并返回结果，使代码更加简洁、易读。
...
![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\msgbox.png)

- 输入框：enterbox

{% asset_img enterbox.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\enterbox.png)

- 选择框：choicebox

{% asset_img choicebox.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\choicebox.png)

- 按钮框：buttonbox

{% asset_img buttonbox.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\buttonbox.png)

- 文件选择框：fileopenbox

{% asset_img fileropenbox.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\fileropenbox.png)
...
![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\class.png)

若属性与方法名相同，属性会覆盖掉方法

{% asset_img same.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\same.png)
...
![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\capsulation.png)
...
- `__init__`(self)：python 类的初始化方法，用来初始化对象的属性，不返回实例。
...
![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\inherit.png)

若子类需在父类属性 / 方法的基础上添加属性 / 方法，需先调用父类的属性 / 方法。例如：super().**()

{% asset_img super.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\super.png)

**多态**：同一种方法名，在不同的子类中有不同的实现，调用时根据对象的类型自动执行对应的方法，实现“一个接口，多种形态”。

{% asset_img polymorphic.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\polymorphic.png)

### 6.3 魔术方法

{% asset_img factory.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\factory.png)

**str**()：给用户看的，追求可读性、简洁易懂；

**`__repr__()`**：给开发者看的，追求精确性、能还原对象（“所见即所得”）。

{% asset_img str_repr %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\str_repr.png)

**eq_**():

{% asset_img eq.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\eq.png)

**属性访问**：

{% asset_img attribute.png %}

![](E:\lanpangzi-blog\source\_posts\python-basic-knowledge-1\attribute.png)

### 6.4 内置函数

```python
if __name__ =="__main__“：表示直接运行该文件，生产环境通常不用，但本地调试非常常见
```

包（package)：一个文件夹内用于存放相关的模块，在该文件夹中必须创建一个"**init**.py"的模块文件，内容可为空
