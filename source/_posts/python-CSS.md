...
网格布局是一种**二维布局**模型，允许开发者通过定义行（rows）和列（columns）来精准控制网页元素的位置和尺寸。还可以实现响应式设计。
...
auto-fit：尽可能**拉伸列填满容器**（会合并空白，列宽不小于 minmax 的最小值）
...
1. 跨列：grid-column:开始线编号 / 结束线编号 **或**  开始线编号 / span 跨单元格数量

2. 跨行：grid-row:开始线编号 / 结束线编号 **或** 开始线编号 / span 跨单元格数量
...
| text        | |型竖线（类似文本输入光标），用于可编辑文本区域（如 textarea、contenteditable 元素） |
